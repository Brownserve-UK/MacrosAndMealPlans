...
use mmp_core::Result;
use mmp_core::domain::{IngredientId, PreparedMealId, Product, ProductId, Revision};
...
macro_rules! columns {
    () => {
        "id, name, brand, barcode, retailer, shopping_section, track_stock, package_quantity_amount, package_quantity_unit, servings_per_pack, mapped_ingredient_id, mapped_prepared_meal_id, nutrition_basis_amount, nutrition_basis_unit, energy_kcal, protein_g, carbohydrate_g, sugar_g, fat_g, saturated_fat_g, fibre_g, salt_g, cholesterol_mg, nutrition_extra, origin, seed_key, source_provider, source_external_id, locally_modified, revision, created_at, updated_at, archived_at"
    };
}

macro_rules! filter {
    () => {
        " WHERE ($1 OR archived_at IS NULL) AND ($2::text IS NULL OR origin = $2) AND ($3::text IS NULL OR name ILIKE '%' || $3 || '%') AND ($4::text IS NULL OR barcode = $4) AND ($5::text IS NULL OR retailer ILIKE '%' || $5 || '%') AND ($6::uuid IS NULL OR mapped_ingredient_id = $6) AND ($7::bool IS NULL OR (mapped_ingredient_id IS NULL) = $7) AND ($8::uuid IS NULL OR mapped_prepared_meal_id = $8)"
    };
}
...
const LIST_ASC: &str = concat!(
    ...
    " FROM product",
    filter!(),
    " ORDER BY CASE WHEN $3::text IS NULL THEN 0 ELSE similarity(name, $3) END DESC, lower(name) ASC LIMIT $9 OFFSET $10"
);
const LIST_DESC: &str = concat!(
    ...
    " FROM product",
    filter!(),
    " ORDER BY CASE WHEN $3::text IS NULL THEN 0 ELSE similarity(name, $3) END DESC, lower(name) DESC LIMIT $9 OFFSET $10"
);
...
const COUNT_BY_PREPARED_MEAL: &str = "SELECT mapped_prepared_meal_id, count(*) FROM product \
     WHERE mapped_prepared_meal_id = ANY($1) AND archived_at IS NULL \
     GROUP BY mapped_prepared_meal_id";
const LIST_BY_PREPARED_MEAL: &str = concat!(
    "SELECT ",
    columns!(),
    " FROM product WHERE mapped_prepared_meal_id = ANY($1) AND archived_at IS NULL \
     ORDER BY mapped_prepared_meal_id, lower(name)"
);
...
impl ProductRepository for PgProductRepository {
    ...
    async fn list(&self, query: &ProductQuery) -> Result<Paginated<Product>> {
        ...
        let mapped_prepared_meal = query.mapped_prepared_meal_id.map(|id| id.as_uuid());
    ...
    async fn count_by_prepared_meal(
        ...
        let uuids: Vec<uuid::Uuid> = prepared_meal_ids.iter().map(|id| id.as_uuid()).collect();
        let rows: Vec<(uuid::Uuid, i64)> = sqlx::query_as(COUNT_BY_PREPARED_MEAL)
            .bind(&uuids)
            .fetch_all(&self.pool)
            .await
            .map_err(|e| repository_error("counting products by prepared meal", e))?;

        let mut counts: HashMap<PreparedMealId, i64> =
            prepared_meal_ids.iter().map(|id| (*id, 0)).collect();
        for (id, count) in rows {
            counts.insert(PreparedMealId::from(id), count);
        }
        Ok(counts)
    }
    ...
    async fn list_by_prepared_meal(
        ...
        let uuids: Vec<uuid::Uuid> = prepared_meal_ids.iter().map(|id| id.as_uuid()).collect();
        let rows: Vec<ProductRow> = sqlx::query_as(LIST_BY_PREPARED_MEAL)
            .bind(&uuids)
            .fetch_all(&self.pool)
            .await
            .map_err(|e| repository_error("listing products by prepared meal", e))?;

        let mut grouped: HashMap<PreparedMealId, Vec<Product>> = HashMap::new();
        for row in rows {
            let product: Product = row.try_into()?;
            if let Some(prepared_meal_id) = product.mapped_prepared_meal_id {
                grouped.entry(prepared_meal_id).or_default().push(product);
            }
        }
        Ok(grouped)
    }
    ...
    async fn insert(&self, product: &Product) -> Result<()> {
        ...
        .bind(product.shopping_section.map(|section| section.code()))
        ...
        .bind(product.mapped_prepared_meal_id.map(|id| id.as_uuid()))
        .execute(&self.pool)
        .await
