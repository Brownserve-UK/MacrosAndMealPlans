...
import { useCreatePreparedMeal } from '../../api/queries';
import { FormDialog } from '../../components/FormDialog';
import { IngredientFields, type IngredientDraft } from '../ingredients/IngredientFields';

const EMPTY: IngredientDraft = { name: '', default_unit: 'item', track_stock: true };

export function NewPreparedMealDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  const navigate = useNavigate();
  const create = useCreatePreparedMeal();
  ...
  async function onSubmit(event: FormEvent) {
    ...
    try {
      ...
      handleClose();
      void navigate({ to: '/prepared-meals/$id', params: { id: created.id } });
    } catch (caught) {
  ...
  return (
    <FormDialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <form onSubmit={onSubmit}>
        <DialogTitle>New prepared meal</DialogTitle>
        <DialogContent>
        ...
        <DialogActions>
          ...
          <Button type="submit" variant="contained" disabled={create.isPending}>
            {create.isPending ? 'Saving…' : 'Create prepared meal'}
          </Button>
        </DialogActions>
      </form>
    </FormDialog>
